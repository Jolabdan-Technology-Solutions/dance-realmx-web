...
export function CoursePreview({ courseId, onEnroll }: CoursePreviewProps) {
  ...
  const createCheckoutSession = useMutation({
    mutationFn: () =>
      apiRequest("/enrollments/checkout", {
        method: "POST",
  ...
  const selectedModule =
    previewModules.find((m) => m.id === selectedModuleId) || null;
              ...
              <div>
                <div className="prose max-w-none">
                    ...
                    <div className="aspect-video bg-muted relative rounded-md overflow-hidden">
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <p className="mt-4 text-muted-foreground">
                          Preview available (limited to {course.previewDuration}{" "}
                          seconds)
                        </p>
                      </div>
                    </div>
                  ...
                  {selectedModule.type === "lesson" && (
                    <div className="border rounded-md p-4 bg-muted/30">
                      <p>
                        This is a sample of the lesson content. The full content
                        is available after enrollment.
                      </p>
                      <p className="mt-2 text-muted-foreground">
                        Preview shows {course.previewPages} page(s).
                      </p>
                    </div>
                  )}
                  ...
                  {selectedModule.type === "quiz" && (
                    <div className="border rounded-md p-4 bg-muted/30">
                      ...
                      <p className="mt-2 text-muted-foreground">
                        Full quiz available after enrollment.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-8 text-center p-4 border border-dashed rounded-md">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Enroll in this course to access all content and get a
