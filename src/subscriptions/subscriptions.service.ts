...
export class SubscriptionsService {
  ...
  async createCheckoutSession(data: {
    ...
    const priceId = data.frequency === 'MONTHLY' 
      ? plan.stripePriceIdMonthly 
      : plan.stripePriceIdYearly;

    if (!priceId) {
      throw new Error(`Stripe price ID for ${data.frequency.toLowerCase()} plan is not set.`);
    }
  ...
  async createStripeSubscriptionSession(user: any, plan: any, priceAmount: number, frequency: 'MONTHLY' | 'YEARLY') {
    if (!user?.email || !plan?.name || !priceAmount) {
      throw new HttpException('Missing required data for Stripe session', HttpStatus.BAD_REQUEST);
    }
    ...
    try {
      ...
      const price = await this.stripe.prices.create({
        ...
        currency: 'usd',
        recurring: {
          interval: frequency.toLowerCase(), // 'month' or 'year'
        },
        product: product.id,
      });
      ...
      throw new HttpException(error.message || 'Failed to create Stripe checkout session', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  

  // Helper to create a pending subscription record
  async createPendingSubscription({ userId, planId, stripeSessionId, status, frequency, currentPeriodStart, currentPeriodEnd }) {
