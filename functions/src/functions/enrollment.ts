...
export const enrollInCourseAdvanced = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
    ...
    try {
      ...
      if (courseData?.price > 0 && !paymentIntentId) {
        throw new HttpsError('invalid-argument', 'Payment required for paid course');
      }
...
export const updateEnrollmentProgress = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    if (!enrollmentId) {
      throw new HttpsError('invalid-argument', 'Enrollment ID is required');
    }
    ...
    try {
      ...
      if (enrollmentData?.userId !== request.auth.uid) {
        throw new HttpsError('permission-denied', 'Cannot update another user\'s enrollment');
      }
      ...
      throw new HttpsError('internal', 'Failed to update enrollment progress');
    }
  }
);

export const getEnrollmentProgress = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    if (!courseId && !enrollmentId) {
      throw new HttpsError('invalid-argument', 'Course ID or Enrollment ID is required');
    }
    ...
    try {
      ...
      if (enrollmentData?.userId !== request.auth.uid) {
        throw new HttpsError('permission-denied', 'Cannot access another user\'s enrollment');
      }
      ...
      throw new HttpsError('internal', 'Failed to get enrollment progress');
    }
  }
);

export const getUserEnrollments = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to fetch user enrollments');
    }
  }
);