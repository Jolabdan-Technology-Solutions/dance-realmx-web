...
export const becomeProfessional = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
    ...
    if (!bio || !danceStyles || !hourlyRate || !location) {
      throw new HttpsError('invalid-argument', 'Bio, dance styles, hourly rate, and location are required');
    }
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to create professional profile');
    }
  }
);

export const searchProfessionals = onCall(
  { cors: true },
  async (request) => {
    const { 
      location,
    ...
    try {
      ...
      if (location && location.coordinates) {
        professionals = professionals.filter((prof: any) => {
          if (!prof.location?.coordinates) return false;
      ...
      throw new HttpsError('internal', 'Failed to search professionals');
    }
  }
);

export const bookProfessional = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    if (!professionalId || !serviceType || !date || !startTime || !duration) {
      throw new HttpsError('invalid-argument', 'Professional ID, service type, date, start time, and duration are required');
    }
    ...
    try {
      ...
      if (!professionalData?.isActive) {
        throw new HttpsError('failed-precondition', 'Professional is not available');
      }
      ...
      for (const booking of conflictingBookings.docs) {
        ...
        if (bookingDateTime < existingEnd && endDateTime > existingStart) {
          throw new HttpsError('resource-exhausted', 'Professional is not available at the requested time');
        }
      }
      ...
      throw new HttpsError('internal', 'Failed to book professional');
    }
  }
);
  ...
  async (request) => {
    ...
    try {
      ...
      if (!canUpdate && !userData?.role?.includes('ADMIN')) {
        throw new HttpsError('permission-denied', 'Cannot update this booking');
      }
      
      // Status-specific validations
      if (status === 'CONFIRMED' && bookingData?.professionalId !== request.auth.uid && !userData?.role?.includes('ADMIN')) {
        throw new HttpsError('permission-denied', 'Only the professional can confirm bookings');
      }
      
      if (status === 'CANCELLED' && bookingData?.clientId !== request.auth.uid && !userData?.role?.includes('ADMIN')) {
        throw new HttpsError('permission-denied', 'Only the client can cancel bookings');
      }
...
export const getProfessionalBookings = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to fetch professional bookings');
    }
  }
);

export const getClientBookings = onCall(
  { cors: true },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to fetch client bookings');
    }
  }
);
