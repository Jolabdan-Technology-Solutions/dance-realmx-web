...
import { 
  PlusCircle, BookOpen, Award, FileBox, 
  Search, Download, Eye,
  Users, UserPlus, Plus, Edit, Loader2
} from "lucide-react";
        ...
        {course.imageUrl && (
          <div className="w-full h-40 overflow-hidden rounded-md mb-4">
            <img 
              ...
              onLoad={() => console.log(`Course image loaded successfully for: ${course.title}`)}
              onError={(e) => {
...
function StudentsList() {
  ...
  const { data: enrollments, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/instructor/enrollments?instructorId=${user?.id}`);
      ...
      {currentStudents.length === 0 ? (
        <div className="bg-muted p-8 rounded-lg text-center">
          ...
          <p className="text-muted-foreground">
            ...
            or no students match your search criteria.
          </p>
        </div>
      ) : (
        <>
                  ...
                  <TableRow key={enrollment.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm whitespace-nowrap">{enrollment.progressPercentage || 0}%</span>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button size="sm" variant="outline" asChild>
                          <Link href={`/instructor/students/${enrollment.studentId}`}>
                            View
                          </Link>
                        </Button>
                        <Button 
...
function CertificatesList() {
  ...
  const { data: certificates, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/instructor/certificates?instructorId=${user?.id}`);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button className="flex gap-2" onClick={() => window.location.href = "/instructor/certificates/issue"}>
          <Plus size={16} />
          Issue Certificate
        </Button>
      </div>
      ...
      {currentCertificates.length === 0 ? (
        <div className="bg-muted p-8 rounded-lg text-center">
          ...
          <p className="text-muted-foreground">
            You haven't issued any certificates yet, or no certificates match your search criteria.
          </p>
          <Button className="mt-4" onClick={() => window.location.href = "/instructor/certificates/issue"}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Avatar className="h-8 w-8">
                          ...
                          <AvatarFallback>
                            {certificate.studentName?.split(' ').map((n: string) => n[0]).join('').toUpperCase() || '??'}
                          </AvatarFallback>
                        </Avatar>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button className="flex gap-2" onClick={() => window.location.href = "/seller/upload-resource"}>
          <Plus size={16} />
          Create Resource
        </Button>
      </div>
      ...
      {currentResources.length === 0 ? (
        <div className="bg-muted p-8 rounded-lg text-center">
          ...
          <h3 className="text-xl font-medium mb-2">No Teaching Resources Found</h3>
          <p className="text-muted-foreground">
            You haven't created any teaching resources yet, or no resources match your search criteria.
          </p>
          <Button className="mt-4" onClick={() => window.location.href = "/seller/upload-resource"}>
            ...
            {currentResources.map((resource: any) => (
              <Card key={resource.id} className="overflow-hidden flex flex-col">
                <div className="h-48 overflow-hidden relative">
                  {resource.imageUrl ? (
                    <img 
                      src={resource.imageUrl.includes('?') ? resource.imageUrl : `${resource.imageUrl}?t=${Date.now()}`} 
                      alt={resource.title} 
                      className="w-full h-full object-cover"
                      onError={(e) => {
                        console.error("Error loading resource image:", resource.title, resource.imageUrl);
                ...
                <CardHeader>
                  ...
                  <CardDescription>{resource.danceStyle || 'All Styles'}</CardDescription>
                </CardHeader>
                <CardContent className="flex-grow">
                  <p className="text-sm line-clamp-3 mb-2">{resource.description}</p>
                  <div className="flex justify-between items-center">
                    <span className="font-semibold text-lg">{formatPrice(resource.price)}</span>
