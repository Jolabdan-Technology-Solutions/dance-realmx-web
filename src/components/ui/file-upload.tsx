...
interface FileUploadProps {
  onUploadComplete: (fileUrl: string | string[], metadata?: FileMetadata | FileMetadata[]) => void;
  defaultValue?: string;
  maxSizeMB?: number;
...
export function FileUpload({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oversizedFiles.length > 0) {
      setError(`${oversizedFiles.length > 1 ? 'Some files exceed' : 'File exceeds'} the maximum size of ${maxSizeMB}MB`);
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!multiple) {
        ...
        if (uploadEndpoint.includes('/profile')) {
          console.log("This is a profile image upload - ensuring proper content type");
          
          // Make sure we're only uploading images for profile
          if (!file.type.startsWith('image/')) {
            throw new Error('Profile images must be image files (JPG, PNG, etc.)');
          }
        }
      } else {
      ...
      if (multiple && data.urls) {
        ...
        console.log("Multiple file upload successful, received URLs:", data.urls);
        ...
        data.urls.forEach((url: string, index: number) => {
          ...
          if (baseUrl.startsWith('/') && !baseUrl.startsWith('//') && typeof window !== 'undefined') {
          ...
          filesMetadata.push({
            ...
            name: data.original_names?.[index] || baseUrl.split('/').pop() || `file-${index}`,
            type: data.mimetypes?.[index] || (files[index]?.type || 'application/octet-stream')
          });
        });
        ...
        const responseUrl = data.url || data.image_url || data.file_url || data.profile_image_url;
        if (!responseUrl) {
        ...
        const fileMetadata: FileMetadata = {
          size: data.size || 0,
          name: data.original_name || baseUrl.split('/').pop() || 'uploaded-file',
        ...
        if (uploadEndpoint.includes('/profile')) {
          ...
          console.log("Sending URL to parent component:", baseUrl, "with size:", data.size);
          onUploadComplete(baseUrl, fileMetadata);
        }
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        {previews.length > 0 && (
          <Button type="button" variant="outline" size="icon" onClick={clearFiles}>
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
      ...
      {previews.length > 0 && (
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
          {previews.map((preview, index) => {
            ...
            if (isImagePreview) {
              return (
                <div key={index} className="relative aspect-video rounded-md overflow-hidden bg-gray-100 dark:bg-gray-800">
                  <img
                    src={preview}
                    alt={`Preview ${index + 1}`}
                    ...
                    onError={(e) => {
                      console.error(`Error loading preview image ${index + 1}:`, preview);
                      e.currentTarget.src = "https://placehold.co/400x400/e2e8f0/a3afc3?text=Image+Error";
                    }}
                  />
                  {multiple && (
              ...
              return (
                <div key={index} className="flex items-center p-2 border rounded-md dark:border-gray-700">
