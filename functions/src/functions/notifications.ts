...
export const getUnreadNotificationCount = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to get unread notification count');
    }
  }
);
  ...
  async (request) => {
    ...
    try {
      ...
      if (notificationData?.userId !== request.auth.uid) {
        throw new HttpsError('permission-denied', 'Cannot mark another user\'s notification as read');
      }
...
export const deleteNotification = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      if (notificationData?.userId !== request.auth.uid) {
        throw new HttpsError('permission-denied', 'Cannot delete another user\'s notification');
      }
...
export const createNotification = onCall(
  ...
  async (request) => {
    ...
    if (!recipientId || !type || !title || !message) {
      throw new HttpsError('invalid-argument', 'Recipient ID, type, title, and message are required');
    }
...
export const sendBulkNotification = onCall(
  ...
  async (request) => {
    ...
    const { 
      recipientIds,
      userFilters, // e.g., { role: 'STUDENT', subscriptionTier: 'PREMIUM' }
      type,
      title,
      message,
      data = {},
    ...
    if (!type || !title || !message) {
      throw new HttpsError('invalid-argument', 'Type, title, and message are required');
    }
    
    if (!recipientIds && !userFilters) {
      throw new HttpsError('invalid-argument', 'Either recipient IDs or user filters must be provided');
    }
    ...
    try {
      ...
      if (recipientIds) {
        ...
        let userQuery: any = admin.firestore().collection('users');
        ...
        targetUserIds = userSnapshot.docs.map((doc: any) => doc.id);
      }
      ...
      if (sendEmail) {
        ...
        const userDocs = await Promise.all(
          targetUserIds.slice(0, 100).map(id => // Limit for demo
