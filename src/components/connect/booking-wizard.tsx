...
import { 
  Calendar as CalendarIcon, ArrowRight, ArrowLeft, User as UserIcon, Check as CheckIcon, 
  Loader2, AlertCircle, Star, Users, Clock, Heart 
} from "lucide-react";

// Mock UI components - replace these with your actual UI library components
...
const Input = ({ className, ...props }) => (
  <input 
    className={`w-full px-3 py-2 rounded-md border focus:outline-none focus:ring-2 focus:ring-blue-500 ${className}`}
    {...props}
  />
);
...
const Alert = ({ children, className }) => (
  <div className={`p-4 rounded-md border-l-4 ${className}`}>
    {children}
  </div>
);
...
const RadioGroupItem = ({ value, id }) => (
  <input type="radio" value={value} id={id} name="radio-group" className="mr-2" />
);

// Calendar component mock
const Calendar = ({ mode, selected, onSelect, disabled, className, classNames }) => (
  <div className={`p-4 ${className}`}>
    <input
      type="date"
      ...
      min={new Date().toISOString().split('T')[0]}
      className="w-full p-2 rounded border bg-black/40 border-white/20 text-white"
    />
  </div>
);
...
const STEP_NAMES = ["Account", "Category", "Location", "Date", "Pricing", "Submit"];
...
export const BookingWizard: React.FC<BookingWizardProps> = ({
  ...
  const apiRequest = useCallback(async (url: string, options: any) => {
    console.log("API Request:", url, options);
    // Mock response for demonstration
    if (url.includes('/professionals/search')) {
      return {
        results: [
          {
            id: 1,
            ...
            bio: "Professional ballet instructor with 10+ years of experience",
            location: "Los Angeles, CA",
            city: "Los Angeles",
            state: "CA", 
            years_experience: 10,
            dance_style: ["Ballet", "Contemporary"],
            service_category: ["instructor"],
            pricing: 75,
            portfolio: "https://sarahdance.com",
            services: ["Private Lessons", "Group Classes"]
          },
          {
            id: 2,
            user: {
            ...
            bio: "Hip hop choreographer and instructor specializing in urban styles",
            location: "Los Angeles, CA",
            city: "Los Angeles", 
            state: "CA",
            years_experience: 8,
            dance_style: ["Hip Hop", "Jazz"],
            service_category: ["choreographer", "instructor"],
            pricing: 60,
            services: ["Choreography", "Private Lessons", "Workshops"]
          }
        ]
      };
    }
  ...
  const lookupZipcode = useCallback(async (zipcode: string) => {
    ...
    setIsZipLookupLoading(true);
    try {
      // Special case for problematic zipcodes
      if (zipcode === "30078") {
        updateFormData("city", "Snellville");
        updateFormData("state", "GA");
        updateFormData("location", "Snellville, GA");
      ...
      if (response.ok) {
        ...
        if (data.places && data.places.length > 0) {
          ...
          updateFormData("city", place["place name"]);
          updateFormData("state", place["state abbreviation"]);
          updateFormData("location", `${place["place name"]}, ${place["state abbreviation"]}`);
        } else {
      ...
      updateFormData("location", "");
      toast({
        title: "Zipcode Lookup Failed",
        description: "Please enter your city and state manually.",
        variant: "destructive",
      });
    } finally {
      setIsZipLookupLoading(false);
  ...
  const validateStep = useCallback((step: number): boolean => {
    switch (step) {
      case 0:
        ...
        if (mode === "get-booked" && (!formData.bio || !formData.phone_number)) {
          toast({
            title: "Required Fields Missing",
            description: "Please fill in your bio and phone number.",
            variant: "destructive",
          });
          return false;
        }
      ...
      case 1:
        if (formData.service_category.length === 0) {
          toast({
            ...
            description: "Please select at least one professional category.",
            variant: "destructive",
          });
          return false;
        }
        if (formData.dance_style.length === 0) {
          toast({
            title: "Dance Style Required", 
            description: "Please select at least one dance style.",
            variant: "destructive",
          });
          return false;
        }
        return true;
      case 2:
        if (!formData.zip_code && !formData.location) {
          toast({
            title: "Location Required",
            description: "Please enter a zipcode or location.",
            variant: "destructive",
          });
          return false;
        }
  ...
  const handleNext = useCallback(() => {
    ...
    setCurrentStep(prev => Math.min(STEP_NAMES.length - 1, prev + 1));
  }, [currentStep, validateStep, user, mode, navigate]);

  const handlePrevious = useCallback(() => {
    setCurrentStep(prev => Math.max(0, prev - 1));
  }, []);

  const transformFormDataToAPI = useCallback((formData: FormData, mode: string, user: User | null) => {
    const baseData = {
      userId: user?.id || user?.user_id || "demo_user",
      serviceCategory: formData.service_category,
      danceStyle: formData.dance_style,
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      let result;
      if (mode === "get-booked") {
        result = await apiRequest("/api/profiles/become-professional", {
          method: "POST",
          data: payload,
          requireAuth: true,
        });
        ...
        // Build search params
        const params = new URLSearchParams();
        formData.service_category.forEach(cat => params.append("service_category", cat));
        formData.dance_style.forEach(style => params.append("dance_style", style));
        if (formData.zip_code) params.append("zip_code", formData.zip_code);
        if (formData.city) params.append("city", formData.city);
        if (formData.state) params.append("state", formData.state);
        if (formData.location) params.append("location", formData.location);
        params.append("travel_distance", formData.travel_distance.toString());
        params.append("pricing", formData.pricing.toString());
        params.append("session_duration", formData.session_duration.toString());

        result = await apiRequest(`/api/profiles/professionals/search?${params.toString()}`, {
          method: "GET",
          requireAuth: true,
        });
      }
      ...
      toast({
        title: mode === "book" ? "Search Complete!" : "Profile Created!",
        description: mode === "book" 
          ? "Found matching professionals in your area."
          : "Your professional profile is now live.",
      });
  ...
  }, [currentStep, validateStep, formData, mode, user, transformFormDataToAPI, apiRequest, toast, onComplete]);

  const handleFavorite = useCallback(async (profileId: number) => {
    const isFavorited = favorites.includes(profileId);
    
    try {
      await apiRequest(`/api/profiles/${profileId}/favorite`, {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {SERVICE_CATEGORIES.map((category) => (
          <Card
            ...
            onClick={() => {
              const updatedCategories = formData.service_category.includes(category.id)
                ? formData.service_category.filter(id => id !== category.id)
                : [...formData.service_category, category.id];
              updateFormData("service_category", updatedCategories);
            }}
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {DANCE_STYLES.map((style) => (
            <div
              key={style.id}
              className={`px-3 py-2 rounded-md cursor-pointer border text-sm flex items-center justify-between backdrop-blur-sm transition-all ${
                formData.dance_style.includes(style.id)
                  ? "border-blue-400 bg-blue-500/20 text-white shadow-lg shadow-blue-500/25"
                  : "border-white/20 bg-black/40 text-white hover:bg-black/60"
              }`}
              onClick={() => {
                const updatedStyles = formData.dance_style.includes(style.id)
                  ? formData.dance_style.filter(id => id !== style.id)
                  : [...formData.dance_style, style.id];
                updateFormData("dance_style", updatedStyles);
              }}
            >
              {style.name}
  ...
  const renderLocationStep = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="pt-4">
          <div className="flex justify-between mb-2">
            <Label className="text-white">Travel Distance</Label>
            <span className="text-sm text-white">{formData.travel_distance} miles</span>
          </div>
          <Slider
            min={5}
            max={100}
            ...
            onValueChange={([value]) => updateFormData("travel_distance", value)}
            className="w-full"
          />
      ...
      <div className="bg-black/40 backdrop-blur-sm p-4 rounded-lg border border-white/20">
        <Calendar
          ...
          classNames={{
            ...
            nav_button: "h-7 w-7 bg-transparent p-0 opacity-100 text-white",
          }}
        />
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="pt-4">
          <div className="flex justify-between mb-2">
            <Label className="text-white">
              {mode === "book" ? "Your budget (hourly rate)" : "Your hourly rate"}
            </Label>
            <span className="text-sm text-white">${formData.pricing}</span>
          </div>
          <div className="h-12 pt-4">
        ...
        {mode === "book" && (
          <div className="mt-4 pt-4 border-t border-white/20">
            ...
            <RadioGroup
              value={formData.session_duration.toString()}
              onValueChange={(value) => updateFormData("session_duration", parseInt(value))}
              ...
              {[
                ...
                <div key={option.value} className="flex items-center space-x-2">
                  <input
                    ...
                    value={option.value}
                    checked={formData.session_duration.toString() === option.value}
                    onChange={(e) => updateFormData("session_duration", parseInt(e.target.value))}
                    className="text-blue-500"
                  />
                  <Label htmlFor={`duration-${option.value}`} className="text-white cursor-pointer">
                    {option.label}
                  </Label>
                </div>
              ))}
  ...
  const renderSubmitStep = () => (
    <div className="space-y-6">
      ...
      {submitSuccess ? (
        mode === "book" && searchResults.length > 0 ? (
            ...
            {searchResults.map((professional) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Heart
                      className={`h-7 w-7 drop-shadow-lg ${
                        isFav ? "fill-pink-500 text-pink-500" : "fill-none text-white"
                      }`}
                      strokeWidth={2}
                    />
                  </button>
                  ...
                  <p className="text-gray-200 text-center mb-2 line-clamp-2 min-h-[48px]">
                    {professional.bio}
                  </p>
                  ...
                  <div className="flex flex-col items-center mb-2">
                    ...
                    <span className="text-xs text-gray-300">
                      {professional.years_experience} yrs experience
                    </span>
                  </div>
          ...
          <div className="text-center space-y-4">
            ...
            <h4 className="text-lg font-medium text-white">
              {mode === "book" ? "Search Complete!" : "Profile Created!"}
            </h4>
            <p className="text-gray-300">
              {mode === "book"
                ? searchResults.length === 0 
                  ? "No professionals found matching your criteria. Try adjusting your search."
                  : "We'll connect you with these professionals soon."
                : "Your professional profile is now live and visible to potential clients."}
            </p>
          </div>
        )
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-black/40 backdrop-blur-sm p-4 rounded-lg border border-white/20 space-y-3">
            <div>
              ...
              <span className="text-gray-300">
                {formData.service_category
                  .map(cat => SERVICE_CATEGORIES.find(c => c.id === cat)?.name)
                  .filter(Boolean)
                  .join(", ")}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="text-gray-300">
                {formData.dance_style
                  .map(style => DANCE_STYLES.find(s => s.id === style)?.name)
                  .filter(Boolean)
                  .join(", ")}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="text-gray-300">
                {formData.location || `${formData.city}, ${formData.state}`}
                {formData.travel_distance && ` (${formData.travel_distance} miles)`}
              </span>
            </div>
            ...
            {mode === "book" ? (
              <>
                <div>
                  <span className="font-medium text-white">Budget: </span>
                  <span className="text-gray-300">${formData.pricing} per hour</span>
                </div>
                <div>
                  <span className="font-medium text-white">Session Duration: </span>
                  <span className="text-gray-300">{formData.session_duration} minutes</span>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  ...
                  <span className="text-gray-300">{formData.years_experience} years</span>
                </div>
          ...
          <p className="text-sm text-gray-300">
            {mode === "book"
              ? "We'll search for matching professionals in your area."
              : "Your professional profile will be visible to clients looking for your services."}
          </p>
        </div>
      )}
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 3: return renderDateStep();
      case 4: return renderPricingStep();
      case 5: return renderSubmitStep();
      default: return null;
    }
  };
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{
          backgroundImage: `url('https://images.unsplash.com/photo-1518611012118-696072aa579a?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80')`,
        }}
      >
        <div className="absolute inset-0 bg-gradient-to-br from-black/80 via-black/60 to-black/80"></div>
      </div>
            ...
            <div className="bg-black/20 backdrop-blur-lg rounded-2xl border border-white/10 shadow-2xl p-6 md:p-8">
              ...
              <div className="mb-8">
                ...
                <div className="mt-4 h-2 bg-white/10 rounded-full overflow-hidden backdrop-blur-sm">
                  <div
                    className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-500 ease-out shadow-lg"
                    style={{ width: `${(currentStep / (STEP_NAMES.length - 1)) * 100}%` }}
                  ></div>
                </div>
              </div>
                  ...
                  !submitSuccess && (
                    ...
                    >
                      {isSubmitting ? (
                        <>
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                          {mode === "book" ? "Searching..." : "Creating Profile..."}
                        </>
                      ) : mode === "book" ? (
                        "Find Professionals"
                      ) : (
